...
/// \brief Creates new empty memspace.
/// \param hMemspace [out] handle to the newly created memspace
/// \return UMF_RESULT_SUCCESS on success or appropriate error code on failure.
///
...
///
/// \details
/// This function duplicates the memory target and then adds its to the memspace
/// This means that original memtarget handle and the handle of the duplicated memtarget are different.
/// and you cannot use it interchangeably. You can use `umfMemspaceTargetGet()` to retrieve new handle.
///
/// \param hMemspace handle to memspace
